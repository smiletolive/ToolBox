...
import com.lib.commui.nova.util.constant.MemoryConstants;
import com.lib.commui.nova.util.constant.TimeConstants;
...
public final class ConvertUtils {
    ...
    /**
     * 字符串用GBK和ISO-8859-1编码
     * @param origin 原始字符串
     * @return 返回编码字符串
     */
    public static String gbkToISO88591String(String origin){
        if (TextUtils.isEmpty(origin)){
            return null;
        }
        try {
            //原始字符串先用字节数组表示，且使用GBK进行编码，然后再用ISO-8859-1编码这个字节数组，最后得到一个字符串
            return new String(origin.getBytes("GBK"), "ISO-8859-1");
    ...
    /**
     * 字符串用ISO-8859-1和GBK编码
     * @param origin 原始字符串
     * @return 返回编码后的字符串
     */
    public static String iso88591ToGBKString(String origin){
        if (TextUtils.isEmpty(origin)){
            return null;
        }
        try {
            //原始字符串先用字节数组表示，且使用ISO-8859-1进行编码，然后再用GBK编码这个字节数组，最后得到一个字符串
            return new String(origin.getBytes("ISO-8859-1"), "GBK");
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
        return null;
    ...
    public static String bytes2HexString(String input) {
        if (TextUtils.isEmpty(input)){
            return input;
        }
        return bytes2HexString(input.getBytes());
    }
